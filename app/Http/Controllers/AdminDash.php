...
class AdminDash extends Controller
{
    ...
    public function rejectVisitRequest(Request $request, $id) {
	    ...
	    public function deleteVisitRequest(Request $request, $id) {
        $visitor = Visitor::find($id);
        $visitor->updated_by = Auth::id();
        $visitor->status = 3;
        $visitor->save();
        ...
        $audit->cid = Auth::id();
        $audit->ip = $_SERVER['REMOTE_ADDR'];
        $audit->what = Auth::user()->full_name.' deleted the visit request for '.$visitor->name.'.';
        $audit->save();

        return redirect('/dashboard/admin/roster/visit/requests')->with('success', 'The visit request has been deleted successfully.');
    }
    ...
    public function viewCalendar() {
        $calendar = Calendar::where('type', '2')->get()->sortByDesc(function($news) {
            return strtotime($news->date.' '.$news->time);
        });
        $news = Calendar::where('type', '1')->get()->sortByDesc(function($news) {
            return strtotime($news->date.' '.$news->time);
        });
    ...
    public function deleteCalendarEvent($id){
        ...
        $audit->cid = Auth::id();
        $audit->ip = $_SERVER['REMOTE_ADDR'];
        $audit->what = Auth::user()->full_name.' removed the news post '.$calendar->title.'.';
        $audit->save();

        return redirect('/dashboard/admin/calendar')->with('success', 'The news posting has been deleted.');
    }
	public function toggleCalendarEventVisibilty($id){
          $calendar = Calendar::find($id);
          $type = '';
